...
which java
nohup java -agentpath:$AGENT_PATH $JAVA_OPS -jar $APP_DIR/TimerTask-1.0-SNAPSHOT.jar --logging.config=classpath:logback-monitor.xml --CUSTOM-ENV=$APP_DIR/timer.properties >/dev/null 2>&1 1>/dev/null &
for((i=1;i<10;i++));do
