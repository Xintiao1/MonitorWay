...
@Controller
@Slf4j
@Api(value = "资产维护", tags = "资产管理维护页面")
public class MwAssetsMainTainController extends BaseApiService {
    ...
    @PostMapping("/mainTain/getHostInfo")
    @ResponseBody
    @ApiOperation("资产维护计划查询主机数据")
    public ResponseBase selectMainTainHostInfo(@RequestBody MwAssetsMainTainParam mainTainParam) {
        Reply reply;
        try {
            reply = mainTainService.selectMainTainHostInfo(mainTainParam);
            if (null != reply && reply.getRes() != PaasConstant.RES_SUCCESS) {
                return setResultFail(reply.getMsg(), reply.getData());
            }
            return setResultSuccess(reply);
        } catch (Exception e) {
            logger.error("资产维护计划查询失败", e);
            return setResultFail("资产维护计划模糊查询失败", "");
        }
    }

    @PostMapping("/mainTain/getModelListInfo")
    @ResponseBody
    @ApiOperation("维护计划查询资产")
    public ResponseBase getModelListInfo(@RequestBody QueryInstanceModelParam param) {
        Reply reply;
        try {
            param.setConvertVal(true);
            reply = mainTainService.getModelListInfo(param);
            if (null != reply && reply.getRes() != PaasConstant.RES_SUCCESS) {
                return setResultFail(reply.getMsg(), reply.getData());
            }
            return setResultSuccess(reply);
        } catch (Exception e) {
            logger.error("资产维护计划查询实例失败", e);
            return setResultFail("资产维护计划查询实例失败", "");
        }
    }
