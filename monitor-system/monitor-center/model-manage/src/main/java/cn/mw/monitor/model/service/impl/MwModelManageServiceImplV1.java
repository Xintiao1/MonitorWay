...
import static cn.mw.monitor.service.model.service.ModelPropertiesType.MULTIPLE_ENUM;
import static cn.mw.monitor.service.model.service.ModelPropertiesType.SINGLE_ENUM;
...
@Slf4j
@Primary
public class MwModelManageServiceImplV1 implements MwModelManageService, MwModelManageCommonService {
    @Value("${datasource.check}")
    private String DATACHECK;
    //纳管模型id
    public static final Integer MANAGEMODELID = 1003;
    public static final String DATEBASEMYSQL = "mysql";
    public static final String DATEBASEORACLE = "oracle";
    private final static int INSERT_MAX_SIZE = 1000;
    @Resource
    ...
    public Reply creatModel(AddAndUpdateModelParam addAndUpdateModelParam) throws IOException {
        ...
        //模型分为 0:内置模型，1:自定义模型
        //内置模型不可删除，超级管理员(userId = 106)可创建内置模型，其余角色暂不可以创建
        if (addAndUpdateModelParam.isBaseModel()) {
            addAndUpdateModelParam.setModelLevel(ModelLevel.BuildIn.getLevel());
            Integer maxBaseModelId = mwModelManageDao.getBaseModelMaxId();
            addAndUpdateModelParam.setModelId(maxBaseModelId + 1);
        } else {
            ...
            if (DATACHECK.equals(DATEBASEORACLE)) {
                synchronized (this) {
                    Integer lastModelId = mwModelManageDao.getManageIdSeq();
                    addAndUpdateModelParam.setModelId(lastModelId);
                }
            }
        }
        if (groupId != -1) {
        ...
        //根据模型视图创建内置属性
        ModelView modelView = ModelView.valueOf(addAndUpdateModelParam.getModelView());
        List<PropertyInfo> propertyInfos = modelView.propertyAdd().add(addAndUpdateModelParam);
        addAndUpdateModelParam.addAllPropertyInfo(propertyInfos);
        addAndUpdateModelParam.setIconType(1);
        //新增数据 pagefield、customcold
        mwModelManageDao.creatModel(addAndUpdateModelParam);
        //
        List<ModelInfo> modelParamList = new ArrayList<>();
        modelParamList.add(addAndUpdateModelParam);
        creatPageFieldAndCustomColData(modelParamList);
    ...
    //获取模型分组下模型的排序大小
    @Override
    public int getModelMaxSort(ModelParam modelParam) {
        int modelSort = mwModelManageDao.getModelMaxSort(modelParam.getModelGroupId());
        int currentModelSort = modelSort + 1;
        return currentModelSort;
    }
     ...
     */
    private void creatPageFieldAndCustomColData(List<ModelInfo> modelParamList) {
        int x = 0;
        List<AddAndUpdateModelPageFieldParam> pageFieldParams = new ArrayList<>();
        for (ModelInfo modelParam : modelParamList) {
            if (CollectionUtils.isNotEmpty(modelParam.getPropertyInfos())) {
                for (PropertyInfo propertyInfo : modelParam.getPropertyInfos()) {
                    x++;
                    AddAndUpdateModelPageFieldParam pageFieldParam = new AddAndUpdateModelPageFieldParam();
                    pageFieldParam.setModelId(modelParam.getModelId());
                    pageFieldParam.setModelPropertiesId(x);
                    pageFieldParam.setProp(propertyInfo.getIndexId());
                    pageFieldParam.setLabel(propertyInfo.getPropertiesName());
                    pageFieldParam.setOrderNumber(propertyInfo.getSort());
                    pageFieldParam.setVisible(propertyInfo.getIsShow());
                    pageFieldParam.setType(propertyInfo.getPropertiesTypeId());
                    pageFieldParams.add(pageFieldParam);
                }
            }
        }
        ...
        if (DATACHECK.equals(DATEBASEORACLE)) {
            synchronized (this) {
                mwModelManageDao.increasePageFieldIdSeq(pageFieldParams.size() + 1);
                Integer lastInstanceId = mwModelManageDao.getPageFieldIdSeq();
                mwModelManageDao.increasePageFieldIdSeq(1);
                Integer start = lastInstanceId - pageFieldParams.size() + 1;
                for (int i = 0; i < pageFieldParams.size(); i++) {
                    AddAndUpdateModelPageFieldParam param = pageFieldParams.get(i);
                    param.setId(start + i);
                }
            }
        }
        //属性新增时，往mw_pagefield_table字段表中同步数据
        if (CollectionUtils.isNotEmpty(pageFieldParams)) {
            mwModelManageDao.batchInsertPropertiesPageField(pageFieldParams);
        }
        ...
        for (AddAndUpdateModelPageFieldParam pageFieldParam : pageFieldParams) {
            for (Integer userId : userIds) {
                ...
                table.setWidth(null);
                table.setVisible(pageFieldParam.isVisible());
                table.setOrderNumber(pageFieldParam.getOrderNumber());
                table.setModelPropertiesId(pageFieldParam.getModelPropertiesId());
                tableList.add(table);
                if (tableList.size() > INSERT_MAX_SIZE) {
                    //数据库为oracle,先调用序列将实体类赋值，再批量插入
                    if (DATACHECK.equals(DATEBASEORACLE)) {
                        //TODO 考虑到序列加锁问题，确保序列唯一
                        synchronized (this) {
                            mwModelManageDao.increaseCustomcoldIdSeq(tableList.size() + 1);
                            Integer lastInstanceId = mwModelManageDao.getCustomcolIdSeq();
                            mwModelManageDao.increaseCustomcoldIdSeq(1);
                            Integer start = lastInstanceId - tableList.size() + 1;
                            for (int i = 0; i < tableList.size(); i++) {
                                MwCustomcolTable param = tableList.get(i);
                                param.setId(start + i);
                            }
                        }
                    }
    ...
    public Boolean createEsIndex(String index) throws IOException {
        ...
        if (!exists) {
            ...
            myAnalyzer.put("filter", filterList);
            normalizer.put("my_analyzer", myAnalyzer);
            analyzer.put("normalizer", normalizer);
...
/*        if(addAndUpdateModelParam.getModelLevel()==0){
    ...
    @Override
    public Reply getAssetsSubTypeByMode() {
        List<Map> mapList = new ArrayList<>();
        try {
            mapList = mwModelAssetsDiscoveryDao.getAssetsSubTypeByMode();
        } catch (Throwable e) {
            log.error("fail to getAssetsSubTypeByMode case by {}", e);
            return Reply.fail("fail to getAssetsSubTypeByMode case by {}", e);
        }
     ...
     */
    @Override
    public Reply getMonitorModeInfo() {
        List<Map> map = mwModelInstanceDao.getMonitorModeInfo();
        return Reply.ok(map);
    }
     ...
     */
    @Override
    public Reply deleteModel(AddAndUpdateModelParam modelParam, Boolean isDelete) throws Exception {
        //先判断是否是内置模型
        log.info("内置未加载");
        //获取当前登录用户ID
        Integer userId = 0;
        ...
        if (userId != 106 && modelParam.getModelLevel() == 0) {
    ...
    void deleteModelById(Integer modelId, String index, Boolean isDelete) throws IOException {
        ...
        //4、删除模型属性pageField、customCol数据
        mwModelManageDao.deleteCustomColByModelId(modelId);
        mwModelManageDao.deletePropertiesPageFieldByModelId(modelId);
    ...
    public Reply selectModelList(ModelParam modelParam) {
        try {
            ...
            for (MwModelManageDto dto : list) {
                ...
                for (UserDTO userDTO : dto.getPrincipal()) {
                    if (userDTO != null) {
                ...
                if (null != dto.getDepartment() && dto.getDepartment().size() > 0) {
                    dto.getDepartment().forEach(department -> {
                                ...
                                if (department != null && department.getNodes() != null) {
...
//            List<ModelManageStructDto> list = mwModelManageDao.queryModelListInfo();
            ...
            List<ModelManageStructDto> structDtoList = new ArrayList<>();
            for (ModelInfo modelInfo : modelInfoLists) {
                ModelManageStructDto dto = new ModelManageStructDto();
                BeanUtils.copyProperties(modelInfo, dto);
                modelInfo.getPropertyInfos();
                List<PropertyInfo> propertyInfoList = new ArrayList<>();
                if (modelAndParentPropertyMap != null && modelAndParentPropertyMap.containsKey(modelInfo.getModelId())) {
                    propertyInfoList = modelAndParentPropertyMap.get(modelInfo.getModelId());
                }

                List<ModelPropertiesStructDto> structList = propertiesStructConvert(propertyInfoList);


                for (ModelPropertiesStructDto structDto : structList) {
                    //结构体数据为9单选和10多选时，将数据String转为lsit数值
                    if ((structDto.getStructType() == SINGLE_ENUM.getCode() || structDto.getStructType() == MULTIPLE_ENUM.getCode()) && (!Strings.isNullOrEmpty(structDto.getStructStrValue()))) {
                        structDto.setStructListValue(Arrays.asList(structDto.getStructStrValue().split(",")));
                    }
                }
                dto.setPropertiesStruct(structList);
                structDtoList.add(dto);
            }
            for (ModelManageStructDto dto : structDtoList) {
    ...
    private List<ModelPropertiesStructDto> propertiesStructConvert(List<PropertyInfo> propertyInfos) {
        ...
        for (PropertyInfo propertyInfo : propertyInfos) {
            ...
            structDto.setStructName(propertyInfo.getPropertiesName());
            structDto.setStructType(propertyInfo.getPropertiesTypeId());
            structDto.setStructId(propertyInfo.getIndexId());
            structDto.setStructStrValue(propertyInfo.getRegex());
            if (SINGLE_ENUM.getCode() == propertyInfo.getPropertiesTypeId() || MULTIPLE_ENUM.getCode() == propertyInfo.getPropertiesTypeId()) {
                structDto.setStructStrValue(propertyInfo.getDefaultValue());
            }
            structList.add(structDto);
        }
        return structList;
    }
    ...
    private void getModelTypeChild(MwModelManageTypeDto
                                           mwModelManageTypeDto, List<MwModelManageTypeDto> mwModelManageTypeDtoList, Set<Integer> modelGroupIdSet) {
        List<MwModelManageTypeDto> childList = new ArrayList<>();
    ...
    private void getChild(MwModelManageDto
                                  mwModelManageDto, List<MwModelManageDto> mwModelManageDtoList, Set<Integer> modelIdSet) {
    ...
    public Reply editorPropertiesSort(List<ModelPropertiesSortParam> param) {
        try {
            ...
            for (ModelPropertiesSortParam p : param) {
                modelIds.add(p.getModelId());
                p.setSort(sort);
                sortMap.put(p.getIndexId(), sort);
                sort += 1;
            }
            //查询模型信息
            List<ModelInfo> modelInfos = mwModelManageDao.selectModelListByIds(new ArrayList<>(modelIds));
            List<AddAndUpdateModelParam> list = new ArrayList<>();
            List<BatchUpdatePropertiesStatusParam> propertyList = new ArrayList<>();
            if (CollectionUtils.isNotEmpty(modelInfos)) {
                for (ModelInfo modelInfo : modelInfos) {
                    //循环模型属性信息
                    for (PropertyInfo propertyInfo : modelInfo.getPropertyInfos()) {
                        if (sortMap != null && sortMap.get(propertyInfo.getIndexId()) != null) {
                            //通过propertiesId和map的key比较，相同则获取排序后的sort
                            propertyInfo.setSort(sortMap.get(propertyInfo.getIndexId()));
                        }
                    }
                }
                ...
                for (ModelInfo modelInfo : modelInfos) {
                    ...
                    list.add(addAndUpdateModelParam);

                    for (PropertyInfo propertyInfo : modelInfo.getPropertyInfos()) {
                        BatchUpdatePropertiesStatusParam propertiesParam = new BatchUpdatePropertiesStatusParam();
                        propertiesParam.setIndexId(propertyInfo.getIndexId());
                        propertiesParam.setModelId(modelInfo.getModelId());
                        propertiesParam.setSort(propertyInfo.getSort());
                        propertyList.add(propertiesParam);
                    }
                }

            }
            mwModelManageDao.batchUpdateModel(list);
            //修改mw_cmdbmd_pagefield_table
            mwModelManageDao.batchUpdatePageFieldSort(propertyList);
            //修改mw_cmdbmd_customcol_table
            mwModelManageDao.batchUpdateCustomcolSort(propertyList);
    ...
    @Override
    @Transactional
    public Reply creatModelProperties(AddAndUpdateModelPropertiesParam propertiesParam) throws Exception {
        ...
        pageFieldParam.setModelPropertiesId(propertiesId);
        ...
        for (Integer userId : userIds) {
            ...
            table.setModelPropertiesId(propertiesId);
            tableList.add(table);
        }
        //数据库为oracle,先调用序列将实体类赋值，再批量插入
        if (DATACHECK.equals(DATEBASEORACLE)) {
    ...
    public Reply updateModelProperties(AddAndUpdateModelPropertiesParam propertiesParam) {
        ...
        if (propertiesParam.getIsShow() != null) {
            //修改mw_cmdbmd_pagefield_table 状态
            mwModelManageDao.updatePropertiesToPageField(propertiesParam);
    ...
    public Reply updateModelPropertiesByGroup(AddAndUpdateModelGroupParam param) {
        ...
        for (ModelInfo modelInfo : modelInfos) {
            if (null != modelInfo.getPropertyInfos()) {
                List<PropertyInfo> propertyInfoList = new ArrayList<>();
                List<PropertyInfo> propertyInfoToPageFieldList = new ArrayList<>();
                propertyInfoList.addAll(modelInfo.getPropertyInfos());
                //获取最大的sort值
                int maxSort = modelInfo.getPropertyInfos().stream().mapToInt(s -> s.getSort() == null ? 0 : s.getSort()).max().orElse(0);
                int sort = maxSort + 1;
                ...
                if (!isFlag) {
                    ...
                    for (int x = 0; x < strName.length; x++) {
                        ...
                        //获取新增的属性，加入到PageField和CustomCo中
                        propertyInfoToPageFieldList.add(propertyInfo);
                    }
                }
                AddAndUpdateModelParam addAndUpdateModelParam = new AddAndUpdateModelParam();
                addAndUpdateModelParam.setModelId(modelInfo.getModelId());
                addAndUpdateModelParam.setModelView(3);//机柜下属设备视图
                String modifier = iLoginCacheInfo.getLoginName();
                addAndUpdateModelParam.setModifier(modifier);
                addAndUpdateModelParam.setPropertyInfos(propertyInfoList);
                mwModelManageDao.updateModel(addAndUpdateModelParam);
                modelInfo.setPropertyInfos(propertyInfoToPageFieldList);
            }
        }
        creatPageFieldAndCustomColData(modelInfos);
        return Reply.ok();
    }
    ...
    private List<ModelPropertiesStructDto> getStructDtoList(AddAndUpdateModelPropertiesParam propertiesParam) {
        ...
        for (ModelPropertiesStructDto modelPropertiesStructDto : propertiesParam.getPropertiesStruct()) {
            ...
            if (modelPropertiesStructDto.getStructType() == SINGLE_ENUM.getCode() || modelPropertiesStructDto.getStructType() == MULTIPLE_ENUM.getCode()) {
    ...
    @Override
    @Transactional
    public Reply deleteModelProperties(List<AddAndUpdateModelPropertiesParam> propertiesParamList) throws
            IOException {
        //如果为内置属性，不可删除
        if (propertiesParamList.size() > 0) {
            ...
            if (null != modelInfo.getPropertyInfos() && !pendingDelId.isEmpty()) {
                ...
                //先通过pagefield_table查询除所有的Customcol_table信息，再删除mw_Customcol_table个性化字段表
                mwModelManageDao.deleteCustomcolTable(modelInfo.getModelId(), new ArrayList<>(pendingDelId));

                //同步删除mw_cmdbmd_pagefield_table
                mwModelManageDao.deletePagefieldTable(modelInfo.getModelId(), new ArrayList<>(pendingDelId));
            }
        }
    ...
    public Reply updateModelPropertiesShowStatus(EditorPropertiesNewParam propertiesParam) {
        ModelInfo modelInfo = mwModelManageDao.selectBaseModelInfoById(propertiesParam.getModelId());
        List<PropertyInfo> propertyInfos = modelInfo.getPropertyInfos();

        for (PropertyInfo propertyInfo : propertyInfos) {
            for (String propertyIndexId : propertiesParam.getPropertiesIndexIds()) {
                if (propertyInfo.getIndexId().equals(propertyIndexId)) {
                    propertyInfo.setIsShow(propertiesParam.getIsShow());
                }
            }
        }
        if (CollectionUtils.isNotEmpty(propertyInfos)) {
            ...
            String modifier = iLoginCacheInfo.getLoginName();
            addAndUpdateModelParam.setModifier(modifier);
            addAndUpdateModelParam.setPropertyInfos(propertyInfos);
            mwModelManageDao.updateModel(addAndUpdateModelParam);
        }

        List<BatchUpdatePropertiesStatusParam> list = new ArrayList<>();
        for (String propertyIndexId : propertiesParam.getPropertiesIndexIds()) {
            BatchUpdatePropertiesStatusParam param = new BatchUpdatePropertiesStatusParam();
            param.setIndexId(propertyIndexId);
            param.setModelId(propertiesParam.getModelId());
            param.setIsShow(propertiesParam.getIsShow());
            list.add(param);
        }
        //同步修改pageField表字段状态
        mwModelManageDao.batchUpdatePageFieldStatus(list);
    ...
    public Reply getPropertiesByManage(Integer modelId) {
        try {
            List<ModelInfo> modelInfos = mwModelManageDao.selectModelListWithParent(MANAGEMODELID);
            ...
            List<String> propertiesIds = propertyInfoList.stream().map(PropertyInfo::getIndexId).collect(Collectors.toList());
        ...
        try {
            ...
            if (CollectionUtils.isNotEmpty(propertyInfoList)) {
                for (PropertyInfo propertyInfo : propertyInfoList) {
                    if (!list.contains(propertyInfo.getPropertiesTypeId()) && propertyInfo.getIsShow()) {
    ...
    /**
     * 查询模型属性的时候，如果模型有父模型，能够查询该模型和父模型的属性
     *
     * @param param
     * @return
     */
    ...
    public Reply selectModelPropertiesList(ModelParam param) {
        try {
            ...
            Map<String, ModelInfo> m = new HashMap();
            for (ModelInfo modelInfo : modelInfoList) {
                if (null != modelInfo.getPropertyInfos()) {
                    allProperties.addAll(modelInfo.getPropertyInfos());
                    for (PropertyInfo propertyInfo : modelInfo.getPropertyInfos()) {
                        m.put(propertyInfo.getIndexId(), modelInfo);
                    }
                }
            }

            List<PropertyInfo> pageInfos = allProperties;
            ...
            for (PropertyInfo propertyInfo : pageInfos) {
                ...
                if (m != null && m.get(propertyInfo.getIndexId()) != null) {
                    ModelInfo modelInfo = m.get(propertyInfo.getIndexId());
                    if (modelInfo.getLevel().intValue() == 3) {
                        modelPropertiesDto.setPropertiesLevelName(modelInfo.getModelName());
                    }
                    modelPropertiesDto.setModelId(modelInfo.getModelId());
                }
                list.add(modelPropertiesDto);
            }
            list = list.stream().sorted(Comparator.comparing(s -> s.getSort())).collect(Collectors.toList());
    ...
    @Override
    public Reply editorPropInfoToPageField() {
        List<ModelInfo> modelInfos = mwModelManageDao.selectAllModelInfo();
        //清空PageField所有数据
        mwModelManageDao.deleteAllPageField();
        //清空CustomCol所有数据
        mwModelManageDao.deleteAllCustomCol();
        creatPageFieldAndCustomColData(modelInfos);
        return Reply.ok();
    }
    ...
    @Transactional
    public void addMapperAndPerm(ModelPermControlParam param) {
     ...
     */
    @Transactional
    public void deleteMapperAndPerm(ModelPermControlParam param) {
