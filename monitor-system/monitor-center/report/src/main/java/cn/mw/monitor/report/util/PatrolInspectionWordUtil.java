...
/**
 * @ClassName PatrolInspectionWordUtil
 * @Description 巡检报表word模板填充工具类
 * @Author gengjb
 * @Date 2022/10/25 14:42
 * @Version 1.0
 **/
@Slf4j
public class PatrolInspectionWordUtil {
    ...
    public static XWPFDocument generateWord(Map<String,Object> param,String template){
        ...
        try {
            ...
            log.error("巡检报告生成word模板失败，失败信息:"+e.getMessage());
        }
        return document;
    }
            ...
            for (XWPFTableRow row : rows) {
                ...
                switch (i){
                    case 0://运行状态数据
                        ...
                        break;
                    case 1://内外核心区
                        deviceDtos = (List<PatrolInspectionDeviceDto>) param.get("coreArea");
                        handleDeviceStatusData(deviceDtos,row,runStatusMap.get("内外核心区"));
                        break;
                    case 2://互联网通道域
                        deviceDtos = (List<PatrolInspectionDeviceDto>) param.get("interNetArea");
                        handleDeviceStatusData(deviceDtos,row,runStatusMap.get("互联网通道域"));
                        break;
                    case 3://外联通道域
                        deviceDtos = (List<PatrolInspectionDeviceDto>) param.get("outreachArea");
                        handleDeviceStatusData(deviceDtos,row,runStatusMap.get("外联通道域"));
                        break;
                    case 4://广域网通道域
                        deviceDtos = (List<PatrolInspectionDeviceDto>) param.get("wideareaNetWork");
                        handleDeviceStatusData(deviceDtos,row,runStatusMap.get("广域网通道域"));
                        break;
                    case 5://外网传统资源池区
                        deviceDtos = (List<PatrolInspectionDeviceDto>) param.get("totalLineOuterNet");
                        handleDeviceStatusData(deviceDtos,row,runStatusMap.get("外网传统资源池区"));
                        break;
                    case 6://内网传统资源池区
                        deviceDtos = (List<PatrolInspectionDeviceDto>) param.get("totalLineIntraNet");
                        handleDeviceStatusData(deviceDtos,row,runStatusMap.get("内网传统资源池区"));
                        break;
                    case 7://综合管理资源池区
                        deviceDtos = (List<PatrolInspectionDeviceDto>) param.get("totalLineSynthesize");
                        handleDeviceStatusData(deviceDtos,row,runStatusMap.get("综合管理资源池区"));
                        break;
                    case 8://带外管理区
                        deviceDtos = (List<PatrolInspectionDeviceDto>) param.get("totalLineOutBand");
                        handleDeviceStatusData(deviceDtos,row,runStatusMap.get("带外管理区"));
                        break;
                    default:
                        break;
                }
