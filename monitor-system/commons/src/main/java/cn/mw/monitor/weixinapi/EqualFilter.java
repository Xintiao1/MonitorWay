...
public class EqualFilter implements MessageFilter{
    MessageContext messageContext;
    MwRuleSelectParam dto;

    public EqualFilter(MessageContext messageContext, MwRuleSelectParam dto) {
    ...
    public boolean filter(MessageContext messageContext) {
        try{
            ...
            if(obj instanceof String){
                return obj.equals(dto.getValue());
            }else if(obj instanceof ArrayList){
                List<String> ruselt = (List<String>) obj;
                for(String s : ruselt){
                    if(s.equals(dto.getValue())){
                        return true;
                    }
                }
            }
