...
public class EndFilter implements MessageFilter{
    MessageContext messageContext;
    MwRuleSelectParam dto;

    public EndFilter(MessageContext messageContext, MwRuleSelectParam dto) {
    ...
    public boolean filter(MessageContext messageContext) {
        try{
            ...
            if(name.contains("指标")){
                ...
                return value.endsWith(result);
            }
            if(obj instanceof String){
                return obj.toString().endsWith(dto.getValue());
            }else if(obj instanceof ArrayList){
                List<String> ruselt = (List<String>) obj;
                for(String s : ruselt){
                    if(s.endsWith(dto.getValue())){
                        return true;
                    }
                }
            }
