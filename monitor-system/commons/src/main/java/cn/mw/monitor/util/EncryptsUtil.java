...
public class EncryptsUtil {
    ...
    public static String key = "EncryptUtil==EncryptUtil";
...
//    }
    ...
    public static String encrypt(String xmlStr) {
        ...
        byte[] md5Hasn = null;
        try {
            md5Hasn = EncryptsUtil.MD5Hash(encrypt, 0, encrypt.length);
        } catch (Exception e) {
            log.error("错误返回 :{}",e);
        }
        // 组合消息体
        byte[] totalByte = EncryptsUtil.addMD5(md5Hasn, encrypt);

        // 取密钥和偏转向量
        byte[] key = new byte[8];
        byte[] iv = new byte[8];
        getKeyIV(EncryptsUtil.key, key, iv);
        ...
        byte[] temp = null;
        try {
            temp = EncryptsUtil.DES_CBC_Encrypt(totalByte, deskey, ivParam);
        } catch (Exception e) {
            log.error("错误返回 :{}",e);
        }
    ...
    public static String decrypt(String xmlStr) throws Exception {
        ...
        byte[] temp = null;
        try {
            temp = EncryptsUtil.DES_CBC_Decrypt(encBuf, deskey, ivParam);
        } catch (Exception e) {
            log.error("错误返回 :{}",e);
        }
        ...
        byte[] md5Hash = null;
        try {
            md5Hash = EncryptsUtil.MD5Hash(temp, 16, temp.length - 16);
        } catch (Exception e) {
            log.error("错误返回 :{}",e);
        }
